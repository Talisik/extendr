...
export class Terminal {
    ...
    constructor(
        ...
        if (process.stdout) process.stdout.on("data", this.#data.push);
    ...
    kill(signal: NodeJS.Signals = "SIGTERM") {
        ...
        if (Config.log) console.debug(`Process '${this.#id}' killed.`);
    }
    ...
    async *listen() {
        for await (const text of this.#listen()) {
            if (Config.log) console.debug(text.substring(0, 32));
    ...
    static async new({
        ...
        spawnConfig?: child_process.SpawnOptions;
    }): Promise<Terminal | null> {
        const now = Date.now();
        const nextRun = Terminal.#lastRun + Terminal.runIntervalMS;
        ...
        if (nextRun > now) {
            ...
            if (Config.log)
                console.debug(`Ran too early. Sleeping for ${sleepTime}ms...`);
        ...
        if (Config.log)
            console.debug(`Running '${command} ${args.join(" ")}'...`);
        ...
        for (let i = 0; i < tryCount; i++) {
            try {
                ...
                const terminal = new Terminal(process, valueSelector);
                let error: any = null;
                ...
                if (Config.log) console.debug("An error occurred.", e);
